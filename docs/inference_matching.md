# Within-subject covariate matching

`dfckit.inference` matches observations from two conditions within the same
participant on continuous covariates. The implementation is condition-
...
```python
from dfckit.inference import match_within_subject
...
```python
from dfckit.inference import matched_subject_differences
