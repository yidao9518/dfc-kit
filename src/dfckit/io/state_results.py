...
from ..states.data import StateAssignments, StateLabelSequence, _readonly
from ..states.metrics import summarize_state_assignments

FORMAT_NAME = "dfckit-state-predictions"
FORMAT_VERSION = 3
READABLE_FORMAT_VERSIONS = frozenset({1, 2, FORMAT_VERSION})
...
class StatePredictions:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "model_seed",
            None if self.model_seed is None else int(self.model_seed),
        )
        if self.model_fingerprint is not None and (
            ...
            raise ValueError("model_fingerprint must be a lowercase SHA-256 hexadecimal string")
        if self.model_fingerprint is not None and self.model_seed is None:
            raise ValueError("model_fingerprint requires a known model_seed")
...
def save_state_predictions(predictions: StatePredictions, path: str | Path) -> Path:
    ...
    if target.exists() or target.is_symlink():
        raise FileExistsError(f"state-prediction artifact path already exists: {target}")
    ...
    try:
        ...
        format_version = (
            3
            if predictions.model_fingerprint is not None
            else 2 if predictions.model_seed is not None else 1
        )
        manifest = {
            "format": FORMAT_NAME,
        ...
        if format_version >= 2:
            manifest["model_seed"] = predictions.model_seed
        if format_version >= 3:
            manifest["model_fingerprint"] = predictions.model_fingerprint
...
def _manifest(root: Path) -> tuple[dict[str, object], dict[str, NDArray]]:
    ...
    try:
        ...
        raise ValueError(f"cannot read state-prediction manifest: {error}") from error
    required = {
        "array_names",
        "format",
        "format_version",
    ...
    expected = required | ({"model_seed"} if version >= 2 else set())
    if version >= 3:
        expected.add("model_fingerprint")
    if set(manifest) != expected:
        raise ValueError("state-prediction manifest fields do not match the schema")
    if manifest["format"] != FORMAT_NAME or version not in READABLE_FORMAT_VERSIONS:
...
def load_state_predictions(path: str | Path) -> StatePredictions:
    ...
    model_seed = (
        ...
        else _positive_integer(manifest["model_seed"], "model_seed", minimum=0)
    )
    model_fingerprint = (
        None if manifest["format_version"] < 3 else manifest["model_fingerprint"]
    )
    expected_arrays = {
        "labels",
...
def write_state_metrics(predictions: StatePredictions, path: str | Path) -> Path:
    ...
    target = Path(path)
    if target.exists() or target.is_symlink():
        raise FileExistsError(f"state-metrics output path already exists: {target}")
    target.parent.mkdir(parents=True, exist_ok=True)
    runs = []
    ...
    payload = {
        ...
        "model_fingerprint": predictions.model_fingerprint,
