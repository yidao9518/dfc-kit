...
class XCPDLowRankIntegrationTests(unittest.TestCase):
    def _write_run(self, root: Path, subject: str, session: str, seed: int) -> None:
        ...
        values = np.column_stack(
            [
                driver + rng.normal(scale=scale, size=32)
    ...
    def test_cli_lowrank_endpoints_feed_generic_paired_inference(self):
        with TemporaryDirectory() as temporary:
            ...
            endpoints = Path(temporary) / "lowrank.json"
            ...
            self.assertEqual(result["n_endpoints"], 13)
            self.assertEqual(result["n_tested"], 13)
