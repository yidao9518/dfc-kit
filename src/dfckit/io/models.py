...
from ..outofcore import StreamingPCAModel
...
def _optional_fingerprint(metadata: Mapping[str, object], name: str) -> str | None:
    value = metadata[name]
    if value is None:
        return None
    if (
        ...
        raise ValueError(f"model metadata {name!r} must be a SHA-256 hexadecimal string")
    return value
...
def fitted_model_fingerprint(model: FittedModel) -> str:
    """Return a deterministic identity for one model's parameters and contract."""
    kind, metadata, arrays = _array_payload(model)
    fingerprint_metadata = dict(metadata)
    for optional_provenance in (
        "init_sample_size",
        "pca_batch_size",
        "training_data_fingerprint",
    ):
        if fingerprint_metadata.get(optional_provenance) is None:
            fingerprint_metadata.pop(optional_provenance, None)
...
def save_fitted_model(model: FittedModel, path: str | Path) -> Path:
    ...
    if target.exists() or target.is_symlink():
        raise FileExistsError(f"model artifact path already exists: {target}")
    target.parent.mkdir(parents=True, exist_ok=True)
    kind, metadata, arrays = _array_payload(model)
    for name, value in arrays.items():
        array = np.asarray(value)
        if array.dtype.hasobject or not np.isfinite(array).all():
            raise ValueError(f"model array {name!r} is not a finite numeric array")
...
def _load_manifest(root: Path) -> tuple[str, dict[str, object], dict[str, NDArray[np.float64]]]:
    ...
    try:
        ...
        raise ValueError(f"cannot read model artifact manifest: {error}") from error
    if not isinstance(manifest, dict):
        raise TypeError("model artifact manifest must be a JSON object")
    ...
    if version == 1:
        legacy_fields = {
            "kmeans-state": _KMEANS_METADATA
            - {"init_sample_size", "training_data_fingerprint"},
            "gaussian-hmm-state": _HMM_METADATA
            - {"pca_batch_size", "training_data_fingerprint"},
            "streaming-pca": _PCA_METADATA,
        }[kind]
        _metadata_fields(metadata, legacy_fields)
        metadata = dict(metadata)
        if kind == "kmeans-state":
            metadata["init_sample_size"] = None
        elif kind == "gaussian-hmm-state":
            metadata["pca_batch_size"] = None
        if kind in {"kmeans-state", "gaussian-hmm-state"}:
            metadata["training_data_fingerprint"] = None
...
def _load_kmeans(
    ...
    return KMeansStateModel(
        ...
        training_data_fingerprint=_optional_fingerprint(
            metadata,
            "training_data_fingerprint",
        ),
